...
showing the value of discriminative calibration. Third, performance is best
when the projection score is used as an auxiliary signal rather than as the
dominant score.

\subsection{Explainability Evaluation}
The deletion test evaluates whether high-contribution explanation terms are
actually influential. Across 158 true label assignments in the test split,
deleting the top-3 contribution terms reduced the mean projection score from
0.4831 to 0.4512, a mean drop of 0.0319. Deleting three random nonzero terms
reduced the mean score to 0.4767, a mean drop of 0.0064. Thus, targeted deletion
produced a score drop about 4.96 times larger than random deletion. This
supports RQ2 by showing that the highlighted terms are not merely descriptive;
they are also influential under the model's scoring function.
