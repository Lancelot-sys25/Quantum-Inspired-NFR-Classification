...
interference strength $\lambda=0$, predicting a label by thresholding the projection score
$s_c(r)=\langle c\mid r\rangle^2 \geq \tau$ (where $\tau \geq 0$) is mathematically equivalent to thresholding the
cosine similarity between $r$ and the class centroid at $\sqrt{\tau}$.
\end{proposition}

\begin{proof}
Since $\lvert r\rangle$ and $\lvert c\rangle$ are unit vectors,
$\langle c\mid r\rangle = \cos\theta_{rc} \geq 0$ because TF-IDF features and positive
centroids are non-negative. Since both sides are non-negative,
$\langle c\mid r\rangle^2 \geq \tau \iff \langle c\mid r\rangle \geq \sqrt{\tau}$.
Therefore, the threshold decision is mathematically equivalent to thresholding the cosine similarity.
\end{proof}
...
These results show that the intrinsic quantum-inspired explanations are highly faithful and competitive with classical attribution baselines.
...
\textbf{Dataset size and availability.} NICE contains only a few hundred NFR-labeled
requirements after filtering. However, multi-label NFR datasets of fine-grained requirements
subclasses are extremely scarce, and NICE is currently the only publicly available dataset
suitable for evaluating multi-label co-occurrence and interference. To address this limitation
and verify generalizability, we also performed cross-dataset validation using the single-label
PROMISE-expanded dataset (525 NFR requirements) as an auxiliary setting, where our model
