...
The main metrics are Micro-F1, Macro-F1, Weighted-F1, Hamming loss, and Label
...
($p=0.125$). Given the small number of folds, these tests should be interpreted
as descriptive evidence rather than a strong statistical conclusion
...
0.6690 to 0.5598, a mean drop of 0.1092. Deleting three random nonzero terms
reduced the mean score to 0.6552, a mean drop of 0.0138. Thus, targeted deletion
produced a score drop about 7.93 times larger than random deletion. This
...
scores. The strongest supported claim is therefore not that a pure
...
substantially more influential than random terms.

Future work should integrate contextual embeddings into the quantum-inspired
representation, compare with fine-tuned BERT/RoBERTa and NoRBERT baselines,
evaluate ML-kNN as a classical multi-label baseline, and collect human rationale
